          ...
          <div className="flex items-center h-14 gap-3">
            ...
            <div className="flex items-center gap-2.5 shrink-0">
              ...
              <div className="hidden sm:block">
                <h1 className="text-sm font-bold tracking-tight text-slate-900 leading-tight flex items-center gap-1">
                  <span>SALA PROVE</span>
                  <span className="text-indigo-600 font-semibold">• {studioInfo.nome}</span>
                </h1>
            ...
            <nav className="flex items-center gap-1.5 flex-1 min-w-0">
              ...
              <div className="relative shrink-0" ref={menuRef}>
                  ...
                  {activeDropdownSection ? (
                    <>
                      ...
                      {activeDropdownSection.badge !== undefined && (
                        <span className="text-[10px] font-bold px-1.5 py-0.5 rounded-full bg-indigo-200/80 text-indigo-900">
                ...
                {/* Dropdown panel */}
                {isMenuOpen && (
                  <div className="absolute left-0 top-full mt-1.5 w-80 sm:w-96 bg-white rounded-xl shadow-xl border border-slate-200 py-2 z-50 animate-in fade-in slide-in-from-top-1 duration-150">
                    <div className="px-3.5 py-2 border-b border-slate-100 flex items-center justify-between">
                      <div className="flex items-center gap-1.5 text-[11px] font-semibold text-slate-500 uppercase tracking-wider">
                        <Layers className="w-3 h-3 text-indigo-500" />
                        <span>Sezioni Gestionali</span>
                      </div>
                      <span className="text-[10px] text-slate-400">{dropdownSections.length} sezioni</span>
                    </div>
                          ...
                          >
                            <div
                              className={`w-7 h-7 rounded-md flex items-center justify-center shrink-0 mt-0.5 ${
                                isItemActive ? 'bg-indigo-100' : 'bg-slate-100'
                              }`}
                            >
                              {item.icon}
                            </div>
                                ...
                                <div className="flex items-center gap-1 shrink-0">
                                  {item.badge !== undefined && (
                                    <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded-full ${
                                      isItemActive ? 'bg-indigo-200 text-indigo-950' : 'bg-slate-100 text-slate-600'
                                    }`}>
                                      {item.badge}
                                    </span>
                                  )}
                                  {isItemActive && <Check className="w-3 h-3 text-indigo-600" />}
                                </div>
                              </div>
            ...
            <div className="flex items-center gap-2 shrink-0">
              <div className="text-[10px] font-mono text-slate-400 bg-slate-50 px-2 py-1 rounded-md border border-slate-200 hidden lg:block leading-tight">
                Orario Continuo: 24/7
              </div>
              <button
                onClick={handleResetDemo}
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-slate-50 hover:bg-slate-100 text-slate-600 hover:text-slate-900 text-xs font-semibold border border-slate-200 transition-colors shadow-sm"
